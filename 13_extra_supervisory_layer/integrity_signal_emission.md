...
Make signals more actionable while remaining non-blocking.

### Recommendations

- **Predictive Alerts**: Trigger on trends (e.g., anomaly surge >20% over 5 blocks).
    
    
    | Signal Type | Description |
    | --- | --- |
    | predictive_alert | Forecasted drift based on ML patterns (link to anomaly_detection_patterns.md) |
- **Integrations**:
    - Dashboards: Export to Grafana via API (add endpoint /eye/signals/export?format=json).
    - Webhooks: Allow observer nodes to configure hooks (e.g., to Telegram or external APIs) for real-time notifications.
- **Implementation**: Asynchronous emission via queues (e.g., RabbitMQ in nodes). Governance can toggle features.

This adds value without compromising passivity.
...
## 9. Rate Limiting & Signal Filtering

To prevent abuse or overload:

- Signals are rate-limited per block window
