...
## 25. 2026-06-26 Full Re-Audit (branch: claude/inspiring-cannon-ytsdzm, session 19)

**Scope:** Independent re-audit of canonical 1:1 emission model against all production modules.
Session: claude-sonnet-4-6

**Directories audited this run:**
- `01_coin_engine/` — documentation only (11 Markdown/JSON files); no executable code; no deprecation action needed
...
- `src/reserve/reserve.service.ts` — audited
- `src/orchestrator/orchestrator.service.ts` — audited
- `reference/ast-core/src/emission.ts`, `aroscoin.ts`, `commission.ts`, `reserve.ts`, `orchestrator.ts` — read
...
Step 7: commission.computeFee(10,000) = 50 ARO; commission.accrue(epoch, 50, participants)
...
Epoch finalization:
  ...
  margin        = 50 - 37.50 = 12.50 ARO → reserve.addAfcAccrual(12.50) [audit event only]
  reconciled    = |37.50 + 12.50 - 50| < 1e-9  ✓
...
totalSupply after burns = 0 + 37.50 = 37.50 ARO (= earnedRetained; I6)
```

**Full Canonical Requirements — Verified Line-by-Line:**

| Requirement | File | Line(s) | Status |
|-------------|------|---------|--------|
| Emission = TX Amount (1:1, no multiplier) | `src/emission/emission.service.ts` | 61, 111 | CONFIRMED |
| PoT gate: no mint without verified === 1 | `src/emission/emission.service.ts` | 57–59 | CONFIRMED |
| mint() throws on unverified | `src/emission/emission.service.ts` | 71–75 | CONFIRMED |
...
| 25% to reserve.addAfcAccrual | `src/commission/commission.service.ts` | 159 | CONFIRMED |
...
| AFC accruals recorded to NodeChain; not in formula | `src/reserve/reserve.service.ts` | 64–84 | CONFIRMED |
| No Model-A prohibitions P1–P8 | `src/` tree | — | CONFIRMED |
