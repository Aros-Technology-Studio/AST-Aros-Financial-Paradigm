...
## 26. 2026-06-28 Full Re-Audit (branch: claude/inspiring-cannon-ctlam9, session 20)

**Scope:** Independent re-audit against the canonical 1:1 emission model.
Session: `session_018NtK3LbTB1BQFxqCfeVzBf` (claude-sonnet-4-6)

**Directories audited this run:**
- `01_coin_engine/` — documentation only; `coin_emission_model.md` and `burn_and_mint_rules.md` already corrected (§9.4/§9.5)
- `10_proof_of_transaction_engine/` — PoT documentation; runtime lives in `src/pot/`
...
- `src/emission/emission.service.spec.ts` — audited (7 tests including `calculate()` suite)
...
| reserveIndex = log10(1 + volume) | `src/reserve/reserve.service.ts` | 92–94 | CONFIRMED |
| AFC accruals recorded, not in formula | `src/reserve/reserve.service.ts` | 64–84 | CONFIRMED |
| Orchestrator: mint → accrue → burn | `src/orchestrator/orchestrator.service.ts` | 162–176 | CONFIRMED |
| No Model-A prohibitions (P1–P8) | `src/` tree | — | CONFIRMED |
