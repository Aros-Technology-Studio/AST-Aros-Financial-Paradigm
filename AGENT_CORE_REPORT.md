...
## 25. 2026-06-26 Full Re-Audit (branch: claude/inspiring-cannon-lzi0g3, session 19)

**Scope:** Independent re-audit of all emission modules against the canonical 1:1 model.
Session: `session_01A2ivk3jFjBqiARga87xegy` (claude-sonnet-4-6)
...
- `src/reserve/reserve.service.ts` — audited
- `src/orchestrator/orchestrator.service.ts` — audited
- `src/invariants/invariants.spec.ts` — all 10 invariants verified
...
## 26. 2026-06-28 Full Re-Audit (branch: claude/inspiring-cannon-txekfx, session 20)
...
- `src/commission/commission.service.ts` — key lines verified (feeRate, marginRate, distributable, addAfcAccrual)
- `src/reserve/reserve.service.ts` — key lines verified (reserveIndex formula, log10, totalProcessVolume)
- `src/orchestrator/orchestrator.service.ts` — lifecycle order verified (mint → commission.accrue → burn)
- `reference/ast-core/src/emission.ts` — canonical reference read
- `reference/ast-core/src/aroscoin.ts` — canonical reference read
...
| burn() mirrors mint (net → 0) | `src/emission/emission.service.ts` | 85–88 | CONFIRMED |
| calculate() pure canonical formula | `src/emission/emission.service.ts` | 107–120 | CONFIRMED |
| Three-tally ledger | `src/aroscoin/aroscoin.service.ts` | 62–80 | CONFIRMED |
| Supply identity (I6): (minted-burned)+earned | `src/aroscoin/aroscoin.service.ts` | 86–89 | CONFIRMED |
| Commission feeRate = 0.005 | `src/commission/commission.service.ts` | 69 | CONFIRMED |
| AFC marginRate = 0.25 (25%) | `src/commission/commission.service.ts` | 72 | CONFIRMED |
| 75% distributable to nodes | `src/commission/commission.service.ts` | 138 | CONFIRMED |
| 25% → reserve.addAfcAccrual | `src/commission/commission.service.ts` | 161 | CONFIRMED |
| reserveIndex = log10(1 + volume) | `src/reserve/reserve.service.ts` | 92–94 | CONFIRMED |
| AFC accruals recorded, not in formula (I-RS-1) | `src/reserve/reserve.service.ts` | 64–84 | CONFIRMED |
| Orchestrator: mint → accrue → burn order | `src/orchestrator/orchestrator.service.ts` | 162–176 | CONFIRMED |
| No Model-A prohibitions (P1–P8) | `src/` tree | — | CONFIRMED |

**Structural Findings:**
- `src/token/` does not exist — confirmed absent. Active emission is in `src/emission/` and `src/aroscoin/`.
- `01_coin_engine/` — documentation only; corrections from §9.4 (reserveIndex formula, code path) confirmed in place.
- `10_proof_of_transaction_engine/` — PoT documentation; no executable content; consistent with Model-1.
- All prior fixes (§4, §9, §15, §19, §20) verified in place.
