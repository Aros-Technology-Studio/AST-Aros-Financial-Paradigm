...
- **Epoch AFC contribution sync** — `FeeDistributionService` distributes AFC on ledger but should call `EmissionService.recordAfcContribution()` after each epoch finalization to keep the in-memory index accurate.
