...
| **2026-06-29** | `claude/inspiring-cannon-heb0vy` | Full re-audit (AGENT-CORE session); canonical 1:1 emission confirmed; §21 added |

---

## 21. 2026-06-29 Full Re-Audit (branch: claude/inspiring-cannon-heb0vy)

**Agent:** AGENT-CORE  
**Directories surveyed:** `01_coin_engine/`, `10_proof_of_transaction_engine/`, `src/token/` (absent),
`src/emission/`, `src/aroscoin/`, `src/commission/`, `src/reserve/`,
`src/orchestrator/`, `reference/ast-core/src/`, `docs/specs/`.  
All files read from scratch; no prior session context assumed.

### Canonical Model — Complete Formula

```
Emission     = Transaction Amount  (1:1, no multiplier; PoT-gated)
Commission   = Transaction Amount × 0.005  (0.5%)
  Node pool  = Commission × 0.75   (75% → nodes post-factum, PoT-confirmed participation weight)
  AFC share  = Commission × 0.25   (25% → Reserve.addAfcAccrual → NodeChain audit event)
Burn         = Emission amount on cycle completion  (processNet → 0)
reserveIndex = log10(1 + totalProcessVolume)  [spec I-RS-1/I-RS-2; AFC accruals are audit-only]
...
|-----------|---------|--------|
| `01_coin_engine/` | Markdown documentation only (11 files). Not deprecated; canonical spec reference. No executable code. | Docs — no action |
| `10_proof_of_transaction_engine/` | PoT documentation only (9 files). Runtime lives in `src/pot/`. | Docs — no action |
| `src/token/` | Does NOT exist. All emission logic lives in `src/emission/` and `src/aroscoin/`. | Absent |
...
| Commission `feeRate = 0.005` | `src/commission/commission.service.ts` | 69 | CONFIRMED |
| AFC `marginRate = 0.25` (25%) | `src/commission/commission.service.ts` | 72 | CONFIRMED |
| 75% distributable to nodes | `src/commission/commission.service.ts` | 134–148 | CONFIRMED |
...
| AFC accruals recorded, not in formula | `src/reserve/reserve.service.ts` | 70–80, 87–89 | CONFIRMED |
| Orchestrator: `mint → accrue → burn` order | `src/orchestrator/orchestrator.service.ts` | lifecycle | CONFIRMED |
| No Model-A prohibitions (P1–P8) | `src/` tree | — | CONFIRMED |
...
→ emission.emit(processId, 10_000):
    ...
    coin.recordBurn(10_000)           [processBurned += 10,000; processNet = 0]
...
Production code fully conforms to the canonical Model-1 specification.
