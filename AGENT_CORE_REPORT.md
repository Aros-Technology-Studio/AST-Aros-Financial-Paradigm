...
## 26. 2026-06-28 Full Re-Audit (branch: claude/inspiring-cannon-ihcz13, session 20)

**Scope:** Independent re-audit of all emission modules against the canonical 1:1 model.
Session: `session_018gJAekNhRjYe2YfZ2DtJmU` (claude-sonnet-4-6)
...
- `src/commission/commission.service.ts` — audited (feeRate, marginRate, finalizeEpoch, addAfcAccrual)
- `src/reserve/reserve.service.ts` — audited (reserveIndex formula, log10 import, totalProcessVolume)
- `src/orchestrator/orchestrator.service.ts` — audited (mint/accrue/burn ordering, lines 162–175)
...
| Emission = TX Amount (1:1) | `src/emission/emission.service.ts` | 61, 111 | CONFIRMED |
| PoT gate: `verified === 1` required | `src/emission/emission.service.ts` | 57–59 | CONFIRMED |
| `mint()` throws without PoT confirmation | `src/emission/emission.service.ts` | 73–74 | CONFIRMED |
| `burn()` mirrors mint (net → 0) | `src/emission/emission.service.ts` | 85–88 | CONFIRMED |
| `calculate()` pure canonical formula | `src/emission/emission.service.ts` | 107–120 | CONFIRMED |
| Commission feeRate = 0.005 | `src/commission/commission.service.ts` | 69 | CONFIRMED |
| AFC marginRate = 0.25 (25%) | `src/commission/commission.service.ts` | 72 | CONFIRMED |
...
| AFC accruals: NodeChain audit only, not in formula | `src/reserve/reserve.service.ts` | 62, 79 | CONFIRMED |
| Orchestrator order: mint → accrue → burn | `src/orchestrator/orchestrator.service.ts` | 162, 172, 175 | CONFIRMED |
| No Model-A prohibitions (P1–P8) | `src/` tree | — | CONFIRMED |
