...
export class BridgeService {
    ...
    async handleFiatDepositWebhook(payload: any, apiKey: string): Promise<any> {
        ...
        try {
            // 4. Canonical 1:1 emission: mint, split 75/25 commission, burn remainder
            const mintResult = await this.tokenService.mintForTransaction(
            ...
            request.relatedTxHash = mintResult.mintTxHash ?? request.externalReference;
            await this.bridgeRepo.save(request);

            this.logger.log(`Deposit processed via Bridge. Ref: ${transactionId}, TX: ${request.relatedTxHash}`);
            return { success: true, txHash: request.relatedTxHash };
