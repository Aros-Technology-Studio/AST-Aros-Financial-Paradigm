...
export class FeeDistributionService {
    ...
    private async distributeRewards(epoch: EpochEntity, totalFees: number, weights: Map<string, number>) {
        ...
        try {
            ...
            // Sync EmissionService in-memory reserve index so getCurrentEmissionPrice()
            // reflects epoch-level contributions, not just per-TX ones.
            this.emissionService.recordAfcEpochContribution(afcReserve);
