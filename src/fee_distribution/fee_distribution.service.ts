...
export class FeeDistributionService {
    ...
    private async distributeRewards(epoch: EpochEntity, totalFees: number, weights: Map<string, number>) {
        ...
        try {
            ...
            // Canonical sync: notify EmissionService of the epoch-level AFC contribution so that
            // the in-memory reserveIndex reflects epoch distributions (not just per-TX emissions).
            this.emissionService.syncAfcReserveFromEpoch(afcReserve, epoch.epochNumber);
