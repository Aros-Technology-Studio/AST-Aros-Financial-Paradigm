 ...
 * aggregate volume of PoT-verified, emitted processes, and underpins internal valuation and
 * Release readiness. It mirrors `reference/ast-core/src/reserve.ts`
 * (`reserveIndex = log10(1 + totalProcessVolume)`), whose `addConfirmedVolume` is fed only the
 * transaction amount of each confirmed process (`reference/ast-core/src/orchestrator.ts`).
 *
 * `totalProcessVolume` is the sum of `emission.minted` snapshots — the process part minted for
 * each PoT-verified process — read from NodeChain, the system of record. Because emission mints
 * only behind the PoT gate, the total grows exclusively from confirmed work (I-RS-1). The index
 * is recomputed from history on every read (I-RS-2), monotonic non-decreasing in volume
 * (I-RS-4), and reflects AST's own capitalization only — not custody (I-RS-3).
 *
 * The AFC reserve share of commission fees is tracked separately (`totalAfcReserve`) as an
 * audit trail; it does not feed `totalProcessVolume` or `reserveIndex` (spec I-RS-1).
 *
...
export class ReserveService {
     ...
     * amounts, the process part minted for each PoT-verified process. Produced only behind the
     * PoT gate, so the total remains confirmed-work volume only (I-RS-1). Recomputed from
     * history on each call (I-RS-2).
     */
