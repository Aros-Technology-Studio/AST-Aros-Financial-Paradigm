 ...
 * (`reserveIndex = log10(1 + totalProcessVolume)`), which feeds `totalProcessVolume` solely from
 * confirmed transaction amounts (`Reserve.addConfirmedVolume(req.amount)` in the reference
 * orchestrator) and never from commission fees.
 *
 * `totalProcessVolume` sums `emission.minted` snapshots — the process part minted for each
 * PoT-verified process — read from NodeChain, the system of record. Because that signal is
 * produced only behind the PoT gate (emission only on verified processes), the total grows
 * exclusively from confirmed work (I-RS-1). The index is recomputed from history on every read
 * (I-RS-2), monotonic non-decreasing in volume (I-RS-4), and reflects AST's own capitalization
 * only — not custody (I-RS-3).
 *
 * The AFC reserve share of commission fees (25 % per epoch, spec `margin_to: Reserve`) is
 * recorded here as an audit trail via `addAfcAccrual`/`totalAfcReserve`, but per the spec formula
 * (`totalProcessVolume = aggregate of tx with verified==1`) it does not enter `reserveIndex`.
 *
...
export class ReserveService {
    ...
    /**
     * Total confirmed-work volume, read from NodeChain history: the sum of `emission.minted`
     * amounts, i.e. the process part minted for each PoT-verified process. Produced only behind
     * the PoT gate, so the total remains confirmed-work volume only (I-RS-1). Recomputed from
     * history on each call (I-RS-2). Commission fees and the AFC reserve share do not
     * contribute here (spec formula: `totalProcessVolume = aggregate of tx with verified==1`).
     */
