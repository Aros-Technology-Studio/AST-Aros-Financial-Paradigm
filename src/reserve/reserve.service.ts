 ...
 *
 * `totalProcessVolume` is fed by exactly one signal: `emission.minted` snapshots — the process
 * part minted for each PoT-verified process. Emission mints only behind the PoT gate, so the
 * total grows exclusively from confirmed work (I-RS-1). The AFC commission share is a fee
 * derived from that same confirmed work, not additional confirmed volume; including it would
 * double-count the underlying process amount, so it is recorded separately via `addAfcAccrual`
 * for audit only and never enters this sum.
 *
