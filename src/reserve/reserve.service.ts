 ...
 *   - `emission.minted`: one snapshot per confirmed process; feeds `totalProcessVolume` and
 *     the `reserveIndex` formula (I-RS-1).
 *   - `reserve.afc.accrual`: one snapshot per finalized epoch's 25% AFC commission share;
 *     tracked for audit purposes (`margin_from: Commission`) but does not enter the
 *     `reserveIndex` formula — the index derives solely from confirmed process volume.
 *
