 ...
 *
 * Two event types are recorded in NodeChain:
 *   - `emission.minted`: one snapshot per confirmed process; this is the sole input to the
 *     reserveIndex formula — confirmed work drives capitalization (I-RS-1).
 *   - `reserve.afc.accrual`: one snapshot per finalized epoch's 25% AFC commission share;
 *     recorded for audit and AFC balance tracking (spec: `margin_from: Commission`),
 *     but does not enter the reserveIndex formula (spec I-RS-1, I-RS-2).
 *
