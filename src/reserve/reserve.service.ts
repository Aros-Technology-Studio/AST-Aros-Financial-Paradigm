 ...
 * readiness. It mirrors `reference/ast-core/src/reserve.ts` exactly
 * (`reserveIndex = log10(1 + totalProcessVolume)`; `totalProcessVolume` fed only by
 * `addConfirmedVolume`, the emitted process amount).
 *
 * `totalProcessVolume` is read from NodeChain `emission.minted` snapshots — the process part
 * minted for each PoT-verified process. Because emission mints only behind the PoT gate
 * (verified === 1), volume grows exclusively from confirmed work (I-RS-1). The AFC reserve
 * share of commission fees (routed via `addAfcAccrual`, spec `margin_to: Reserve`) is recorded
 * to NodeChain as its own audit signal and does not feed `totalProcessVolume` or `reserveIndex`.
 *
...
export class ReserveService {
    ...
    /**
     * Total confirmed-work volume, read from NodeChain history: the sum of every
     * `emission.minted` snapshot — the process part minted for each PoT-verified process.
     * Produced only behind the PoT gate, so the total remains confirmed-work volume only
