 ...
 * (`reserveIndex = log10(1 + totalProcessVolume)`) and the spec
 * (`totalProcessVolume = aggregate of tx with verified==1`).
 *
 * `totalProcessVolume` is fed solely by `emission.minted` snapshots — the process part minted
 * for each PoT-verified process — read from NodeChain, the system of record. Because emission
 * only mints behind the PoT gate, the total grows exclusively from confirmed work (I-RS-1).
 * The AFC reserve share (25 %) of commission fees is routed here separately by CommissionService
 * (spec `margin_to: Reserve`) and tracked via `totalAfcReserve()` as an audit trail; it does not
 * enter `totalProcessVolume` or `reserveIndex` (I-RS-1).
...
export class ReserveService {
    ...
    /**
     * Total confirmed-work volume, read from NodeChain history. Sums `emission.minted` — the
     * process part minted for each PoT-verified process — which is produced only behind the
     * PoT gate, so the aggregate remains confirmed-work volume only (I-RS-1). The AFC reserve
     * share of commission fees is tracked separately via `totalAfcReserve()` and does not
     * contribute here. Recomputed from history on each call (I-RS-2).
     */
