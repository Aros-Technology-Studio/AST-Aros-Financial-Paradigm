...
export class TokenController {
     ...
     * Mint = txAmount, Fee = txAmount × rate (75% nodes / 25% AFC), Burn after TX.
     */
    @Post('emit')
    async emitCanonical(@Body() body: { transactionAmount: number; recipient: string; referenceId: string; commissionRate?: number }) {
        try {
