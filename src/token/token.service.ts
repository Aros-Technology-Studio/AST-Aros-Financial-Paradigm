...
export class TokenService {
    ...
    /**
     * Legacy entry point — now redirects to the canonical 1:1 emission flow.
     * All ARO emission must go through: Mint → FeeDistribution(75/25) → Burn.
     */
    async mint(amount: string, recipient: string, referenceId: string): Promise<any> {
        const txAmount = parseFloat(amount);
        if (txAmount <= 0) throw new BadRequestException('Amount must be positive');

        this.logger.log(
            `[Legacy mint → Canonical] TX=${referenceId} amount=${amount} recipient=${recipient}`,
        );

        const result = await this.mintForTransaction(txAmount, recipient, referenceId);
        await this.smartContractService.recordReference(referenceId, 'MINT', { amount, recipient });

        return {
            status: 'SUCCESS',
            txHash: referenceId,
            emissionAmount: result.emissionAmount.toFixed(8),
            commission: result.commission.toFixed(8),
            nodeShare: result.nodeShare.toFixed(8),
            afcReserveShare: result.afcReserveShare.toFixed(8),
        };
    }
